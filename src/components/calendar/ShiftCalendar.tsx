...
// カレンダーの会社一覧を保持するための変数（コンポーネント外で定義）
...
const TimeTreeEvent = ({
    ...
    const company = companiesData?.find((c: Company) => c.id === event.companyId);

    return (
        <div
            className="text-xs px-1 py-0.5 truncate rounded mb-1 font-medium"
            style={{
                backgroundColor: company?.color || "#3B82F6",
                opacity: 0.8,
                color: "#ffffff", // テキストの色を白に変更して視認性を高める
            }}
        >
            {event.title}
        </div>
...
const getCompanyColorClass = (companyId: string, companies: Company[] = []) => {
    // 会社の設定された色を検索
...
const ShiftCalendar = ({ companies = [] }: ShiftCalendarProps) => {
        ...
        (events: CalendarEvent[], companies: Company[], date: Date) => {
            ...
            companies.forEach((company) => {
                ...
                companyStatsMap[company.id] = {
                    ...
                    workingHours: 0,
                    totalWage: 0,
                    colorClass: getCompanyColorClass(company.id, companies),
                };
            });
            ...
            monthEvents.forEach((event) => {
                ...
                if (company?.hourly_wage) {
                    ...
                    companyStatsMap[event.companyId].totalWage +=
                        durationHours * company.hourly_wage;
                }
            ...
            if (error) {
                ...
                const calendarEvents = (data || []).map((shift: Shift) => {
                    ...
                    return {
                        ...
                        companyColor: getCompanyColorClass(shift.company_id, companies),
                    };
                });
    ...
    const handleShiftAdded = (newShift?: Shift) => {
        ...
        // 新しいシフトが渡された場合、eventsに直接追加して即時UI更新
        if (newShift) {
            const company = companies.find((c) => c.id === newShift.company_id);
            const companyName = company ? company.name : "Unknown";

            // 日付と時間からDateオブジェクトを作成
            const [year, month, day] = newShift.date.split("-").map(Number);
            const [startHour, startMinute] = newShift.start_time.split(":").map(Number);
            const [endHour, endMinute] = newShift.end_time.split(":").map(Number);
            ...
            const newEvent: CalendarEvent = {
                id: newShift.id,
                title: `${companyName} ${newShift.start_time} - ${newShift.end_time}`,
                start,
                end,
                resource: newShift,
                companyId: newShift.company_id,
                companyColor: getCompanyColorClass(newShift.company_id, companies),
            };
            ...
            // 月次統計を再計算
            calculateMonthlyStats([...events, newEvent], companies, currentDate);
        } else {
            // 従来通りのリロード（フォールバック）
            fetchShifts();
        }
                        ...
                        <div className="flex flex-wrap gap-2">
                            {companyStats.map((company) => (
                                <div
                                    ...
                                    style={{
                                        backgroundColor: getCompanyColorClass(company.id, companies),
                                        opacity: 0.8,
                                        color: "#ffffff"
                                    }}
                                >
                                    ...
                                    <div className="flex flex-col text-sm">
                                        ...
                                        <span>¥{company.totalWage.toLocaleString()}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
